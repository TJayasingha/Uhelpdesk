...
from helpdesk.models import (Ticket, Queue, FollowUp, Attachment, IgnoreEmail, TicketCC,
                             CustomField, TicketCustomFieldValue, TicketDependency,QueueMembers)
...
class AbstractTicketForm(CustomFieldMixin, forms.Form):
    ...
    ticket_type = forms.ChoiceField(
        ...
        initial='1',
        label=_('Type'),
        # help_text=_("Ticket Type is default to Request Please changed it as your requirements"),
    )
...
#     ''' Adds Members to a Queue. Used for processing POST requests. '''

#     class Meta:
#         model = QueueMembers
#         fields ["queue","member"]
        # exclude = ('QueueMembers',)

    # def __init__(self, *args, **kwargs):
    #     super(QueueMemberForm, self).__init__(*args, **kwargs)
    #     queus = Queue.objects.filter(slug=request.GET.get('queue', None))
    #     self.fields['queue'].queryset = queus